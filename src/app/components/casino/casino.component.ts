...
import { FilterPipe } from './app.filtre';
...
ngOnInit(): void {
  ...
  GetALL100Casino(page,limit,urlca){
    page+=1
  ...
  addItems(startIndex, endIndex) {
    this.GetALL100Casino(this.page,endIndex,this.urlinit)
  }
  onScroll(e) {
    if( (e.currentScrollPosition>this.scrollLM)&&(this.bolScroll==false)){
      this.scrollLM+=3000;
      this.bolScroll=false;
      this.load=false
     this.spinner.show()
      this.GetALL100Casino(this.page,this.limitCasinoScroll,this.urlinit)
  }
   else if(this.limitCasinoScroll>=2826) {
    this.spinner.hide()
      return;
    }
  }
